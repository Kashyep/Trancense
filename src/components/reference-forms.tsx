...
export function EquipmentForm({ workspaceId, facilityId }: { workspaceId: string; facilityId: string }) {
  ...
  return <form action={action} className="utility-card p-6">
    ...
    <Button className="mt-1" disabled={pending}>{pending ? "Saving…" : "Add equipment"}</Button>{state.ok && <p role="status" className="mt-3 text-sm text-[#816729]">Equipment saved.</p>}{state.error && <p role="alert" className="field-error mt-3">{state.error}</p>}
  </form>;
}
...
export function EnergyImportForm({ workspaceId, auditId, boundaryId }: { workspaceId: string; auditId: string; boundaryId: string }) {
  ...
  return <form action={action} className="utility-card p-6">
    ...
    <Button className="mt-1" disabled={pending}>{pending ? "Importing…" : "Import monthly data"}</Button>{state.ok && <p role="status" className="mt-3 text-sm text-[#816729]">Imported {state.rows ?? 0} draft record(s).</p>}{state.error && <p role="alert" className="field-error mt-3">{state.error}</p>}
  </form>;
}
