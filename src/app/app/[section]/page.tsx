...
export default async function WorkspacePage({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (section === "energy") {
    ...
    content = <div className="space-y-8"><FindingForm workspaceId={c.workspaceId} auditId={c.audit.id} evidence={evidence ?? []} /><section className="data-card p-6"><h2 className="text-3xl">Review queue</h2>{findings?.length ? <ul className="mt-6 divide-y divide-[var(--border)]">{findings.map(finding => <li className="py-5" key={finding.id}><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">{finding.category || "Observation"} · {finding.confidence || "Unrated"} confidence</p><h3 className="mt-2 text-2xl">{finding.title}</h3><p className="mt-3 max-w-3xl text-[var(--foreground)]">{finding.observation}</p></div><FindingReviewButtons workspaceId={c.workspaceId} auditId={c.audit!.id} recordId={finding.id} state={finding.review_state} /></div></li>)}</ul> : <EmptyState title="No findings yet">Create the first observation after reviewing the evidence and energy inputs.</EmptyState>}</section></div>;
    ...
    content = <div className="space-y-8"><RecommendationForm workspaceId={c.workspaceId} auditId={c.audit.id} findings={findings ?? []} /><section className="data-card p-6"><h2 className="text-3xl">Action state machine</h2><p className="mt-2 muted">Each action advances one state at a time. Reviewer-only transitions are visibly separated from execution steps.</p>{recommendations?.length ? <ul className="mt-6 divide-y divide-[var(--border)]">{recommendations.map(item => <li className="py-5" key={item.id}><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">{item.affected_system || "Action"} · {item.priority || "Unprioritized"}</p><h3 className="mt-2 text-2xl">{item.intervention}</h3><p className="mt-2 text-sm muted">{item.finding_id ? `Finding: ${findingTitles.get(item.finding_id) || "Linked finding"}` : "No linked finding"} · {item.estimated_savings_kwh ? `${Number(item.estimated_savings_kwh).toLocaleString("en-IN")} kWh estimated` : "No savings estimate"}</p></div><div className="text-right"><Badge tone={item.state === "verified" ? "good" : "neutral"}>{item.state.replaceAll("_", " ")}</Badge><div className="mt-3"><RecommendationAdvanceButton workspaceId={c.workspaceId} auditId={c.audit!.id} recommendationId={item.id} state={item.state as RecommendationState} role={c.role} /></div></div></div></li>)}</ul> : <EmptyState title="No recommendations yet">Turn a reviewed finding into an accountable action with assumptions, risks, and an owner.</EmptyState>}</section></div>;
    ...
    content = <div className="space-y-8"><section className="data-card p-6"><h2 className="text-3xl">Draft report export</h2><p className="mt-3 muted">Exports use authorized persisted audit data and label unavailable, estimated, and unverified information.</p><div className="mt-5"><ReportForm workspaceId={c.workspaceId} auditId={c.audit.id} canGenerate={["owner", "reviewer"].includes(c.role)} /></div></section><section className="data-card p-6"><h2 className="text-3xl">Export history</h2>{reports?.length ? <ul className="mt-5 divide-y divide-[var(--border)]">{reports.map(report => <li className="flex flex-wrap items-center justify-between gap-4 py-4" key={report.id}><div><p className="font-medium">Trancense draft report · v{report.version}</p><p className="mt-1 text-sm muted">{report.status} · {dateLabel(report.generated_at || report.created_at)}</p></div>{report.status === "completed" && <Link href={`/api/reports/${report.id}`} className="link-ember text-sm">Download securely</Link>}</li>)}</ul> : <p className="mt-5 text-sm muted">No exports yet. Generate one after reviewing the audit inputs.</p>}</section></div>;
  } else {
